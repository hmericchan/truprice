...
<input style={{…style,paddingRight:value?30:style.padding||“7px 10px”}} value={value} onChange={e=>onChange(e.target.value)} {…props}/>
{value&&(
<button onMouseDown={e=>{e.preventDefault();onChange(””);}} style={{position:“absolute”,right:8,background:“none”,border:“none”,cursor:“pointer”,fontSize:14,color:”#aaa”,padding:0,lineHeight:1}}>×</button>
)}
</div>
);
}
...
<div style={{position:“relative”,display:“flex”,alignItems:“center”}}>
<input style={{…style,paddingRight:value?30:style.padding||“7px 10px”}} value={value}
onChange={e=>{onChange(e.target.value);setOpen(true);}}
onFocus={()=>setOpen(true)} placeholder={placeholder}/>
{value&&(
<button onMouseDown={e=>{e.preventDefault();onChange(””);setOpen(false);}} style={{position:“absolute”,right:8,background:“none”,border:“none”,cursor:“pointer”,fontSize:14,color:”#aaa”,padding:0,lineHeight:1}}>×</button>
)}
</div>
...
{payload.map((p,i)=>{
// Find entries for this key on this date
...
return displayCurrency===“HKD”?toHKD:toHKD*(fxRates[displayCurrency]||1);
}
...
origPrice:e.origPrice?String(e.origPrice):””,note:e.note||””,priceDate:e.priceDate||today(),currency:e.currency||“HKD” });
setEditId(e.id); setTab(“add”);
}
function handleDuplicate(e) {
setForm({ name:e.name,brand:e.brand||””,store:e.store||””,pricingType:e.pricingType||“single”,
price:String(e.price),qty:String(e.qty),unit:e.unit,bundleQty:String(e.bundleQty||2),
origPrice:e.origPrice?String(e.origPrice):””,note:e.note||””,priceDate:today(),currency:e.currency||“HKD” });
...
return parseFloat((min*0.9).toFixed(1));
},[chartData,chartKeys]);
  ...
  {tab==="chart"&&(
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14,alignItems:"center"}}>
        ...
        {["item","brand","store"].map(g=>(
          <button key={g} onClick={()=>setChartGroupBy(g)} style={{...toggleBtn(chartGroupBy===g),textTransform:"capitalize"}}>{g}</button>
        ))}
      </div>
      {chartData.length===0
